...
def fetch_candles(client, end_time=None, limit=200):
    params = {
        "symbol": "BTCUSDT",
        "granularity": "5min",
        "limit": limit,
    ...
    return client._make_request("GET", "/api/v2/spot/market/history-candles", params=params)
...
def main():
    ...
    output_path = Path("5-min-BTC.csv")
    ...
    max_batches = int(os.getenv("MAX_BATCHES", "400"))
    batch_count = 0
    ...
    while True:
        ...
        if code == "429":
            print("Rate limit hit; sleeping 5 seconds before retry.")
            time.sleep(5)
            continue
        ...
        for candle in candles:
            # Expected order: [timestamp, open, high, low, close, baseVolume, quoteVolume]
            ts = int(candle[0])
            if ts in seen:
                continue
            seen.add(ts)
        ...
        # Prepare for next request: move end_time to oldest candle minus one interval
    ...
    with output_path.open("w", newline="") as fh:
        ...
        writer.writerow(["timestamp_ms", "iso_time", "open", "high", "low", "close", "base_volume", "quote_volume"])
        for row in all_rows:
            ts_seconds = row["timestamp_ms"] / 1000 if row["timestamp_ms"] > 1_000_000_000 else row["timestamp_ms"]
